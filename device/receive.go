...
func (device *Device) DeterminePacketTypeAndPadding(packet []byte, expectedType uint32) (uint32, int) {
	...
	// For fixed-size handshake packets: padding = size - expected_size (O(1)).
	if expectedType == MessageUnknownType || expectedType == MessageInitiationType {
		if device.headers.init != nil {
			p := size - MessageInitiationSize
			if p >= 0 && p <= device.paddings.init && p < size {
				if device.headers.init.Validate(binary.LittleEndian.Uint32(packet[p:])) {
					return MessageInitiationType, p
				}
			}
		}
	}
	...
	if expectedType == MessageUnknownType || expectedType == MessageResponseType {
		if device.headers.response != nil {
			p := size - MessageResponseSize
			if p >= 0 && p <= device.paddings.response && p < size {
				if device.headers.response.Validate(binary.LittleEndian.Uint32(packet[p:])) {
					return MessageResponseType, p
				}
			}
		}
	}
	...
	if expectedType == MessageUnknownType || expectedType == MessageCookieReplyType {
		if device.headers.cookie != nil {
			p := size - MessageCookieReplySize
			if p >= 0 && p <= device.paddings.cookie && p < size {
				if device.headers.cookie.Validate(binary.LittleEndian.Uint32(packet[p:])) {
					return MessageCookieReplyType, p
				}
			}
		}
	}

	// For variable-size transport packets: scan padding positions 0..max (O(max_padding)).
	if expectedType == MessageUnknownType || expectedType == MessageTransportType {
		if device.headers.transport != nil {
			for p := 0; p <= device.paddings.transport; p++ {
				if size >= p+MessageTransportHeaderSize {
					if device.headers.transport.Validate(binary.LittleEndian.Uint32(packet[p:])) {
						return MessageTransportType, p
					}
				}
			}
		}
