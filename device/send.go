...
func (peer *Peer) SendHandshakeInitiation(isRetry bool) error {
	...
	if maxPadding := peer.device.paddings.init; maxPadding > 0 {
		padding := int(fastrandn(uint32(maxPadding))) + 1
...
func (peer *Peer) SendHandshakeResponse() error {
	...
	if maxPadding := peer.device.paddings.response; maxPadding > 0 {
		padding := int(fastrandn(uint32(maxPadding))) + 1
		buf := make([]byte, padding+len(packet))
		rand.Read(buf[:padding])
		copy(buf[padding:], packet)
...
func (device *Device) SendHandshakeCookie(initiatingElem *QueueHandshakeElement) error {
	...
	if maxPadding := device.paddings.cookie; maxPadding > 0 {
		padding := int(fastrandn(uint32(maxPadding))) + 1
		buf := make([]byte, padding+len(packet))
		rand.Read(buf[:padding])
		copy(buf[padding:], packet)
...
func (peer *Peer) RoutineSequentialSender(maxBatchSize int) {
	...
	for elemsContainer := range peer.queue.outbound.c {
		...
		for _, elem := range elemsContainer.elems {
			...
			if maxPadding := device.paddings.transport; maxPadding > 0 {
				padding := int(fastrandn(uint32(maxPadding))) + 1
